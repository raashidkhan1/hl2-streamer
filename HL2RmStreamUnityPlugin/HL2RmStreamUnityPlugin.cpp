...
void __stdcall HL2Stream::Initialize()
{
	...
	m_cameraReader = std::make_shared<RMCameraReader>(m_pLTSensor, camConsentGiven, &camAccessCheck, guid);
	OutputDebugStringW(L"HL2Stream::cameraReader initialized: Done.\n");
...
void HL2Stream::InitializeResearchModeProcessing()
{
	...
	auto LTStreamer = std::make_shared<ResearchModeFrameStreamer>(L"23941", guid, m_worldOrigin);
	...
	if (m_pLTSensor) {
		auto processor = std::make_shared<ResearchModeFrameProcessor>(
			m_pLTSensor, camConsentGiven, &camAccessCheck, 0, m_pLTStreamer);
...
BYTE* __stdcall HL2Stream::GetLongThrowDepthData(int* dataSize)
{
	...
	BYTE* dataWithTimestamp = nullptr;
	...
	if (m_cameraReader->m_spSensorFrame)
	{

		// Check if the frame is a depth frame
		HRESULT hr = m_cameraReader->m_spSensorFrame->QueryInterface(IID_PPV_ARGS(&m_pDepthFrame));
		...
		if (m_pDepthFrame && SUCCEEDED(hr))
		{
			...
			if (spDepthFrame) {
				depthPgmData = m_cameraReader->GetLongThrowDepthData(m_cameraReader->m_spSensorFrame, spDepthFrame);
				...
				size_t totalSize = sizeof(long long) + depthPgmData.size();

				// Allocate memory for timestamp + depth data
				dataWithTimestamp = new BYTE[totalSize];

				// Copy timestamp to the beginning of dataWithTimestamp
				std::copy(reinterpret_cast<BYTE*>(&timestamp),
					reinterpret_cast<BYTE*>(&timestamp) + sizeof(long long),
					dataWithTimestamp);

				// Copy depth data after the timestamp
				std::copy(depthPgmData.begin(),
					depthPgmData.end(),
					dataWithTimestamp + sizeof(long long));
...
void __stdcall HL2Stream::FreeUnmanagedMemory(BYTE* data) {
	if (data) {
		delete[] data;
		OutputDebugStringW(L"FreeUnmangedMemory ");
	}
}

long long HL2Stream::GetCurrentTimestamp() {
	long long ticksPerDay = 864000000000;
	long long epochOffsetDays = 719162;
	long long epochOffsetTicks = epochOffsetDays * ticksPerDay;

	auto now = std::chrono::system_clock::now();
	auto duration = now.time_since_epoch();
	auto ticks = std::chrono::duration_cast<std::chrono::duration<long long, std::ratio_multiply<std::hecto, std::nano>>>(duration).count();

	return ticks + epochOffsetTicks;
}