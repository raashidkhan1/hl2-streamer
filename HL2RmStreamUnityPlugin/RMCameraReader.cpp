    ...
    if (SUCCEEDED(hr))
    {
        ...
        while (!pCameraReader->m_fExit && pCameraReader->m_pRMSensor)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (pSensorFrame) {
                    std::shared_ptr<IResearchModeSensorFrame> m_pSensorFrame(pSensorFrame, [](IResearchModeSensorFrame* sf) { sf->Release(); });
                    pCameraReader->m_spSensorFrame = m_pSensorFrame;
                }
            }
        }
...
// Modified function to return only long throw depth data
std::vector<BYTE> RMCameraReader::GetLongThrowDepthData(std::shared_ptr<IResearchModeSensorFrame> spSensorFrame, std::shared_ptr<IResearchModeSensorDepthFrame> spDepthFrame)
{
    ResearchModeSensorResolution resolution;
    spSensorFrame->GetResolution(&resolution);
    ...
    if (isLongThrow)
    {
        winrt::check_hresult(spDepthFrame->GetSigmaBuffer(&pSigma, &outSigmaBufferCount));
    }

    winrt::check_hresult(spDepthFrame->GetBuffer(&pDepth, &outDepthBufferCount));

    // Create header and data for Depth
    //const std::string depthHeaderString = CreateHeader(resolution, 65535);
    ...
    woss << L"Height size: " << imageHeight << L"\n";
    OutputDebugString(woss.str().c_str());

    woss << L"Width size: " << imageWidth << L"\n";
    OutputDebugString(woss.str().c_str());

    depthPgmData.reserve(outDepthBufferCount * sizeof(UINT16));

    assert(outDepthBufferCount == outSigmaBufferCount);

    // Validate depth and populate data vector
